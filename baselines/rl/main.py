...
This module runs a DQN training loop between a shared RL model and the
SUMO traffic light environment.
"""
...
from baselines.rl.agent import TrafficAgent
...
def _build_sumo_cmd(
    ...
    sumo_cfg_path: os.PathLike[str] | str | None = None,
) -> list[str]:
...
def evaluate_model(
    ...
    sumo_cfg_name: str | None = None,
    scale: float | None = 0.5,
    results_filename: str = "cologne3_low_results.xlsx",
    demand_label: str = "LOW",
) -> pd.DataFrame:
    """Evaluate the trained model on one network for 30 fixed seeds."""
    config = Config()
    ...
    sumo_cfg_path = network_dir / resolved_sumo_cfg_name
    results_path = output_dir / results_filename
    evaluation_seeds = list(range(30))
    epsilon = 0.0
    action_dim = 2
    metric_columns = [
        "cars_passed",
...
def main() -> None:
    ...
    try:
        ...
        state_dim = max(len(v) for v in state_dict.values())
        action_dim = 2
        model = TrafficModel(state_dim=state_dim, action_dim=action_dim)
        agent = TrafficAgent(env=env, model=model, config=config)
        global_step = 0
        ...
        for episode in range(start_episode, config.num_episodes):
            ...
            for step in range(config.max_steps):
                if step % config.action_interval == 0:
                    for tls_id in tls_ids:
                        action_dict[tls_id] = agent.act(state_dict[tls_id], state_dim)
                ...
                for tls_id in tls_ids:
                    padded_prev = _pad_state_vector(previous_state_dict[tls_id], state_dim)
                    padded_next = _pad_state_vector(next_state_dict[tls_id], state_dim)
                    agent.learn(
                        (
                            padded_prev,
                            action_dict[tls_id],
            ...
            print(
                ...
                f"epsilon={agent.epsilon:.3f} teleports={episode_metrics['teleports']:.0f} "
