...
DEFAULT_MAIN_RESULTS_ROOT = REPO_ROOT / "benchmark/results/independent_dqn_v2"
ALLOWED_MAIN_NETWORKS = {"cologne1", "ingolstadt1", "ingolstadt7", "cologne8", "bologna_pasubio", "arterial4x4", "grid4x4", "toronto"}
...
def run_main(
    *,
    ...
    np.random.seed(seed)
    if str(network) not in ALLOWED_MAIN_NETWORKS:
        raise RuntimeError(
            f"Main v2 runner is locked to {sorted(ALLOWED_MAIN_NETWORKS)} only, got: {network}"
        )

    preexisting_model_network_dirs = (
        {p.name for p in main_model_root.iterdir() if p.is_dir()} if main_model_root.exists() else set()
    )
    preexisting_main_run_dirs = (
        {p.name for p in main_run_root.iterdir() if p.is_dir() and p.name.startswith("independent_dqn_v2_main__")}
        if main_run_root.exists()
        else set()
    )
    ...
    expected_episodes = int(
        (
            ((contract.get("fixed_compute_budget_policy", {}) or {}).get("per_network_episode_budgets", {}) or {})
        ).get(
            str(network),
            int(episodes),
        )
    )
    per_network_compute_budget = (
        ((contract.get("fixed_compute_budget_policy", {}) or {}).get("per_network_compute_budget", {}) or {})
    )
    expected_max_steps = int(
        (per_network_compute_budget.get(str(network), {}) or {}).get(
    ...
    if int(episodes) != expected_episodes:
        raise RuntimeError(
            f"Main v2 for {network} must use episodes={expected_episodes} per locked contract, got {episodes}"
        )
    if int(max_steps_per_episode) != expected_max_steps:
        raise RuntimeError(
            f"Main v2 for {network} must use max_steps_per_episode={expected_max_steps} per locked contract, got {max_steps_per_episode}"
        )
    ...
    network_model_dir = main_model_root / network
    checkpoint_source_rel = f"benchmark/models/independent_dqn_v2/{network}/"
    ...
    for ep in range(1, int(episodes) + 1):
        ...
        label = f"idqn_v2_main_train_{int(time.time() * 1_000_000)}_{ep}"
        ...
        try:
            with outputs["sumo_stdout"].open("w", encoding="utf-8") as sumo_stdout:
                ...
                if controller is None:
                    ...
                    if not startup_config_printed:
                        print(
                            (
                                "[idqn_v2_main] startup "
                                f"network={network} "
                                f"episodes={int(episodes)} "
                                f"max_steps_per_episode={int(max_steps_per_episode)} "
                                f"tls_agent_count={int(len(agent_ids))} "
                                f"train_every_steps={int(train_every_steps)} "
                                f"learning_starts_steps={int(learning_starts_steps)} "
                                f"output_dir={network_model_dir}"
                            ),
                            flush=True,
                        )
                        startup_config_printed = True
        ...
        cumulative_wall_s = max(time.perf_counter() - train_wall_start, 0.0)
        avg_ep_wall_s = cumulative_wall_s / max(ep, 1)
        episodes_remaining = max(int(episodes) - int(ep), 0)
        eta_s = avg_ep_wall_s * episodes_remaining
        latest_episode = episode_summaries[-1] if episode_summaries else {}
        print(
            (
                "[idqn_v2_main] progress "
                f"episode={int(ep)}/{int(episodes)} "
                f"episode_wall_s={float(latest_episode.get('episode_wall_time_s', 0.0)):.3f} "
                f"cumulative_wall_s={cumulative_wall_s:.3f} "
                f"eta={_format_duration_s(eta_s)} "
                f"episode_reward={float(latest_episode.get('episode_reward_sum', 0.0)):.6f} "
                f"gradient_updates_so_far={int(total_gradient_updates)} "
                f"invalid_actions={int(invalid_action_count)} "
                f"forbidden_traci_calls={int(total_forbidden_calls)}"
            ),
            flush=True,
        )

    if controller is None:
        raise RuntimeError("Main training did not initialize controller.")
    ...
    episode_loss_means = [
        float(ep["episode_loss_mean"])
        for ep in episode_summaries
        if ("error" not in ep and ep.get("episode_loss_mean") is not None)
    ]
    loss_episode_first10_mean = _mean(episode_loss_means[:10])
    loss_episode_last10_mean = _mean(episode_loss_means[-10:])
    _loss_dir, loss_episode_trend_delta, loss_episode_trend_delta_pct = _trend_label(
        loss_episode_first10_mean,
        loss_episode_last10_mean,
        flat_pct_threshold=5.0,
    )
    ...
    training_summary = {
        ...
        "loss_first_10_episode_mean": loss_episode_first10_mean,
        "loss_last_10_episode_mean": loss_episode_last10_mean,
        "loss_trend_delta": loss_episode_trend_delta,
        "loss_trend_delta_pct": loss_episode_trend_delta_pct,
    ...
    training_manifest = {
        ...
        "train_every_steps": int(train_every_steps),
        "learning_starts_steps": int(learning_starts_steps),
        "observation_version": "v2 aggregate lane-level",
        "reward_version": "aggregate_local_efficient_pressure_v2",
        "forbidden_traci_call_count": int(total_forbidden_calls),
        "target_update_interval": int(target_update_interval),
        "full_main_training_launched": True,
        "full_90_run_evaluation_launched": False,
        "output_files": {
            "training_main_manifest_json": str(network_model_dir / "training_manifest.json"),
            "training_main_summary_json": str(network_model_dir / "training_summary.json"),
    ...
    _json_dump(network_model_dir / "training_summary.json", training_summary)
    _json_dump(network_model_dir / "training_manifest.json", training_manifest)
    ...
    for scenario_id in scenario_ids:
        ...
        run_id = f"independent_dqn_v2_main__{scenario_id}"
        run_dir = main_run_root / run_id
        if run_dir.exists():
            import shutil
        ...
        label = f"idqn_v2_main_eval_{int(time.time() * 1_000_000)}"
        port = _allocate_traci_port()
    ...
    # Extraction outputs
    main_results_root.mkdir(parents=True, exist_ok=True)
    per_run_csv = main_results_root / f"main_{network}_per_run.csv"
    per_run_json = main_results_root / f"main_{network}_per_run.json"
    ...
    summary_csv = main_results_root / f"main_{network}_summary_by_network_level.csv"
    summary_json = main_results_root / f"main_{network}_summary_by_network_level.json"
    ...
    # Validation checks
    per_agent_checkpoint_count = len([p for p in network_model_dir.glob("agent_*.pt") if p.is_file()])
    model_network_dirs = (
        sorted([p.name for p in main_model_root.iterdir() if p.is_dir()]) if main_model_root.exists() else []
    )
    run_dirs = sorted([p for p in main_run_root.iterdir() if p.is_dir()]) if main_run_root.exists() else []
    target_run_dirs = [p for p in run_dirs if p.name.startswith(f"independent_dqn_v2_main__{network}__")]
    post_model_network_dir_set = set(model_network_dirs)
    post_main_run_dir_set = {p.name for p in run_dirs if p.name.startswith("independent_dqn_v2_main__")}
    new_model_network_dirs = sorted(post_model_network_dir_set - preexisting_model_network_dirs)
    new_main_run_dirs = sorted(post_main_run_dir_set - preexisting_main_run_dirs)
    full_horizon_parity_ok = all(bool(r["horizon_parity_ok"]) for r in eval_results)
    kpi_populated = all(
    ...
    trend_summary_exists = (
        training_summary.get("reward_first_10_mean") is not None
        and training_summary.get("reward_last_10_mean") is not None
        and training_summary.get("reward_trend_delta") is not None
        and training_summary.get("loss_first_10_episode_mean") is not None
        and training_summary.get("loss_last_10_episode_mean") is not None
        and training_summary.get("loss_trend_delta") is not None
    )

    expected_tls_agents = (
        8 if str(network) == "cologne8" else int(len(adapter.static_cache.tls_specs))
    )

    checks = {
        "main_trainer_imports": True,
        ...
        "main_training_completes_expected_episodes": len(episode_summaries) == int(episodes) and all(
        ...
        "one_agent_and_checkpoint_per_tls": (
            int(len(agent_ids)) == int(len(adapter.static_cache.tls_specs))
            and per_agent_checkpoint_count == int(len(adapter.static_cache.tls_specs))
        ),
        ...
        "reward_loss_trend_summary_exists": trend_summary_exists,
        "wall_clock_and_projected_runtime_recorded": (
            all("episode_wall_time_s" in ep for ep in episode_summaries)
            and training_summary.get("projected_500_episode_runtime_s") is not None
        ),
        "all_9_evaluations_pass": all(r["run_status"] == "evaluation_passed" for r in eval_results),
        "full_horizon_parity_passes": full_horizon_parity_ok,
        "extraction_per_run_9_rows": len(per_run_rows) == 9,
        "extraction_summary_3_rows": len(summary_rows) == 3,
        "all_4_kpis_populated": kpi_populated,
        "no_other_network_was_trained": all(n == network for n in new_model_network_dirs),
        "no_full_90_run_evaluation_launched": (
            len(target_run_dirs) == 9
            and all(f"__{network}__" in p.name for p in target_run_dirs)
            and all(f"__{network}__" in name for name in new_main_run_dirs)
        ),
    }
    checks["overall_result_pass"] = all(checks.values())

    log_summary_json = REPO_ROOT / f"benchmark/logs/independent_dqn_v2_main_{network}_summary.json"
    log_summary_txt = REPO_ROOT / f"benchmark/logs/independent_dqn_v2_main_{network}_summary.txt"
    log_validation_txt = REPO_ROOT / f"benchmark/logs/independent_dqn_v2_main_{network}_validation.txt"

    _json_dump(log_summary_json, {"main_summary": main_summary, "checks": checks, "torch": torch_status})

    summary_lines = [
        f"Independent DQN v2 {network} Main Summary",
        "",
        ...
        f"Projected 500-episode runtime (s): {training_summary['projected_500_episode_runtime_s']}",
        f"Projected 500-episode runtime (h): {training_summary['projected_500_episode_runtime_h']}",
        ...
        f"Loss first 10 episode mean: {training_summary['loss_first_10_episode_mean']}",
        f"Loss last 10 episode mean: {training_summary['loss_last_10_episode_mean']}",
        f"Loss trend: delta={training_summary['loss_trend_delta']}, delta_pct={training_summary['loss_trend_delta_pct']}",
        f"Loss first updating-window mean: {training_summary['loss_first_updating_window_mean']}",
        f"Loss last updating-window mean: {training_summary['loss_last_updating_window_mean']}",
        f"9-scenario evaluation all passed: {main_summary['evaluation']['all_9_passed']}",
        f"Overall validation: {'PASS' if checks['overall_result_pass'] else 'FAIL'}",
    ]
    log_summary_txt.write_text("\n".join(summary_lines) + "\n", encoding="utf-8")

    vlines = [
        f"Independent DQN v2 {network} Main Validation",
        "",
        f"1. main trainer imports: {'PASS' if checks['main_trainer_imports'] else 'FAIL'}",
        ...
        f"6. main training completes {int(episodes)} episodes: {'PASS' if checks['main_training_completes_expected_episodes'] else 'FAIL'}",
        f"7. {int(expected_tls_agents)} TLS agents are detected: {'PASS' if checks['tls_agents_detected_expected'] else 'FAIL'}",
        f"8. one agent/checkpoint exists per TLS: {'PASS' if checks['one_agent_and_checkpoint_per_tls'] else 'FAIL'}",
        f"9. replay buffer receives transitions: {'PASS' if checks['replay_buffer_receives_transitions'] else 'FAIL'}",
        ...
        f"14. forbidden TraCI call count = 0: {'PASS' if checks['forbidden_traci_calls_zero'] else 'FAIL'}",
        f"15. reward/loss trend summary exists: {'PASS' if checks['reward_loss_trend_summary_exists'] else 'FAIL'}",
        f"16. wall-clock timing and projected runtime are recorded: {'PASS' if checks['wall_clock_and_projected_runtime_recorded'] else 'FAIL'}",
        f"17. all 9 frozen {network} evaluations pass: {'PASS' if checks['all_9_evaluations_pass'] else 'FAIL'}",
        f"18. horizon parity passes for all 9 evaluations: {'PASS' if checks['full_horizon_parity_passes'] else 'FAIL'}",
        f"19. per-run extraction has 9 rows: {'PASS' if checks['extraction_per_run_9_rows'] else 'FAIL'}",
        f"20. summary extraction has 3 rows: {'PASS' if checks['extraction_summary_3_rows'] else 'FAIL'}",
        f"21. all 4 KPIs are populated: {'PASS' if checks['all_4_kpis_populated'] else 'FAIL'}",
        f"22. no other network was trained: {'PASS' if checks['no_other_network_was_trained'] else 'FAIL'}",
        f"23. no full 90-run evaluation launched: {'PASS' if checks['no_full_90_run_evaluation_launched'] else 'FAIL'}",
        f"24. overall result = {'PASS' if checks['overall_result_pass'] else 'FAIL'}",
        "",
    ...
    return {
        ...
        "main_runs_root": str(main_run_root),
        "main_results_root": str(main_results_root),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Independent DQN v2 single-network main runner.")
    ...
    parser.add_argument("--main-model-root", type=Path, default=DEFAULT_MAIN_MODEL_ROOT)
    parser.add_argument("--main-run-root", type=Path, default=DEFAULT_MAIN_RUN_ROOT)
    parser.add_argument("--main-results-root", type=Path, default=DEFAULT_MAIN_RESULTS_ROOT)
    parser.add_argument("--seed", type=int, default=17)
    args = parser.parse_args()

    # Enforce contract settings for this main.
    if int(args.train_every_steps) != 8 or int(args.learning_starts_steps) != 1000:
        raise RuntimeError("Main must use train_every_steps=8 and learning_starts_steps=1000")
    ...
    result = run_main(
        ...
        main_model_root=args.main_model_root,
        main_run_root=args.main_run_root,
        main_results_root=args.main_results_root,
        seed=int(args.seed),
    )
